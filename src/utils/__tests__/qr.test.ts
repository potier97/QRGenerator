import { hexToRgb, rgbToHex, formatQRContent, QRType } from '../qr'
import '@testing-library/jest-dom'
import { describe, it, expect } from '@jest/globals'

describe('Utilidades de QR', () => {
  describe('hexToRgb', () => {
  ...
  describe('formatQRContent', () => {
    it('formatea correctamente URLs', () => {
      expect(formatQRContent('url' as QRType, 'example.com')).toBe('https://example.com')
      expect(formatQRContent('url' as QRType, 'https://example.com')).toBe('https://example.com')
    })

    it('formatea correctamente emails', () => {
      expect(formatQRContent('email' as QRType, 'test@example.com')).toBe('mailto:test@example.com')
    })

    it('formatea correctamente números de teléfono', () => {
      expect(formatQRContent('phone' as QRType, '+34123456789')).toBe('tel:+34123456789')
    })

    it('formatea correctamente ubicaciones', () => {
      expect(formatQRContent('location' as QRType, 'Madrid, España'))
        .toBe('geo:0,0?q=Madrid%2C%20Espa%C3%B1a')
    })

    it('formatea correctamente configuraciones WiFi', () => {
      expect(formatQRContent('wifi' as QRType, 'Mi Red WiFi'))
        .toBe('WIFI:T:WPA;S:Mi Red WiFi;P:;;')
    })

    it('mantiene el texto plano sin cambios', () => {
      expect(formatQRContent('text' as QRType, 'Hola Mundo')).toBe('Hola Mundo')
    })
  })
}) 